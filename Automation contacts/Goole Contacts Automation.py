...
def add_contact(name: str, phone_number: str, current_course: str, credentials: Credentials, contacts_window: ContactsWindow) -> None:
    ...
    if not is_contact_exists:
        ...
        if response.status_code == HTTP_OK_STATUS:
            print(f"Contact '{get_display(name)}' added successfully.")
            contacts_window.add_contact(name)
            contacts_window.contacts_list.update() 
            QCoreApplication.processEvents()         
        else:
            print(f"Failed to add contact '{get_display(name)}'. \nError: {response.text}")

        msg_box = QMessageBox()
        msg_box.setText(f"Contact '{name}' added successfully.")
